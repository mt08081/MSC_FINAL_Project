...
## Update 4
Implementation for run-time done. At first, I added logic where calculations were happening for each agent. The households called the `check-happiness=and-move` function and this function further called the `relocate` function when certain conditions regarding tolerance were met. An agent moves whenever the choosen parameter's pct of the region is lower than the tolerance threshold. Then, demographic vitalities are updated. This includes births and deaths per 1000. This is converted into a percentage and then the `hatch` or `die` commands are used. Finally, `update-visualization` is called to make sure visual simulation changes are made for observational purposes.

This was working fine operationally. However, in terms of computation and efficiency, this was terrible. This was when I decided to use "Yearly Census Counts" which are carried out by `managers`. This meant that each household won't have to count every single time. This allowed for the household operations to go from $O(n^2)$ to $O(n)$.
